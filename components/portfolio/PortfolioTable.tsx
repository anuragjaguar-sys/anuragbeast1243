...
export default function PortfolioTable({
  ...
  const items = portfolio.filter((item) => item.type === type);
        ...
        <table className="min-w-full">
          <thead>
            <tr className="border-b border-zinc-800">
              ...
              <th className="px-4 py-3 text-left text-sm text-zinc-400">Category</th>
              <th className="px-4 py-3 text-left text-sm text-zinc-400">
                {type === "Asset" ? "Growth Rate" : "Interest Rate"}
              </th>
              <th className="px-4 py-3 text-right text-sm text-zinc-400">Value</th>
              <th className="px-4 py-3 text-center text-sm text-zinc-400">Actions</th>
            </tr>
          </thead>
          <tbody>
            {items.length === 0 && (
            ...
            {items.map((item) => {
              const rateDisplay =
                item.type === "Asset"
                  ? `${item.expectedReturn ?? 0}% CAGR`
                  : `${item.interestRate ?? 0}% p.a.`;
              ...
              return (
                ...
                >
                  ...
                  <td className="px-4 py-4 text-zinc-300">{item.category}</td>
